...
/**
 * A suffix tree from 2 strings and finding their longest common substring
 * <p>
 * From https://sites.google.com/site/indy256/algo/suffix_tree
 */
public class SuffixTree {
    private static final String ALPHABET = "abcdefghijklmnopqrstuvwxyz0123456789\1\2";
    ...
    public Node buildSuffixTree(CharSequence s) {
        int n = s.length();
        byte[] a = new byte[n];
    ...
    public static void main(String[] args) {
        ...
        for (int step = 0; step < 100_000; step++) {
            ...
            String s = s1 + '\1' + s2 + '\2';
            Node tree = st.buildSuffixTree(s);
            lcsLength = 0;
            lcsBeginIndex = 0;
            // find longest common substring
            st.lcs(tree, s1.length(), s1.length() + s2.length() + 1);
    ...
    public int lcs(Node node, int i1, int i2) {
